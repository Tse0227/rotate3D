...
* @Last Modified time:  2018-06-21 19:36:29
...
var rotate3D = {
	...
	get_absolute_rotate_vector : function(e) {
		...
		this.currentLocation = {x : e.clientX, y : e.clientY};
		let dx = e.clientX - this.lastLocation.x;
		let dy = e.clientY - this.lastLocation.y;
	...
	click : function() {
		document.addEventListener("click", function(e) {
			rotate3D.isRotate = rotate3D.isRotate ? false : true;
		})
		return this;
	}, 
	//鼠标移动事件
	mousemove : function() {
		document.addEventListener("mousemove", function(e) {
			if(rotate3D.isRotate) {
				rotate3D.get_absolute_rotate_vector(e);//获取相对于窗口坐标系的绝对旋转轴
				rotate3D.get_relative_rotate_vector();//获取相对于容器的坐标系的相对旋转轴
				rotate3D.get_relative_rotate_matrix();//获取相对于容器的坐标系的相对旋转矩阵
				rotate3D.get_absolute_rotate_matrix();//获取相对于窗口坐标系的绝对旋转矩阵
				rotate3D.set_rotate_matrix();
			}
		})
	}
}
